...
export default function IdentityValidation() {
  ...
  // Supporte paginator {data,...} ou simple array
  const items = useMemo(
    () => (Array.isArray(participants) ? participants : (participants?.data ?? [])),
    [participants]
  );

  // IDs masqués localement après action (optimistic UI)
  ...
  const handleValidate = (id) => {
    ...
    router.patch(route('admin.identity.approve', id), {}, {
      preserveScroll: true,
      onSuccess: () => hideId(id),     // enlève la ligne tout de suite
  ...
  return (
    <AppLayout title="Validation des identités">
      ...
      <div className="p-6 bg-white text-[#1B1B18]">
        ...
        {/* Modale document */}
        {modalUser && (
          <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center" onClick={() => setModalUser(null)}>
            <div className="bg-white p-6 rounded-lg shadow-lg w-[90%] max-w-xl relative" onClick={(e)=>e.stopPropagation()}>
              <button
