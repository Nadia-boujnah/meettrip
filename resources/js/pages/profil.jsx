...
/**
 * pickActivityImage
 * -> Choisit la bonne image pour une activité :
 *    1) URL absolue (http/https) envoyée par le back
 *    2) Chemin storage Laravel (/storage/…)
 *    3) Fallback vers un placeholder
 */
...
/**
 * pickAvatar
 * -> Photo de profil si dispo, sinon image par défaut.
 */
const pickAvatar = (u) => u?.photo || '/images/default-avatar.png';

export default function ProfilPublic() {
  // Je récupère depuis Inertia : le user, ses activités, le flag "isOwner", et le compteur
  const { props } = usePage();
  const {
    user = null,
    activities = [],
  ...
  // État de sécurité : si pas d’utilisateur, j’affiche une page neutre
  if (!user) {
    return (
      <AppLayout>
        <Head title="Profil" />
  ...
  // Avatar calculé à partir des données user
  const avatar = pickAvatar(user);

  return (
    <AppLayout>
      {/* Titre de l’onglet : je personnalise avec le nom/prénom */}
      <Head title={`Profil de ${user?.prenom ?? ''} ${user?.nom ?? ''}`} />

      <div className="max-w-5xl mx-auto px-6 py-8 space-y-10">

        {/* En-tête du profil : avatar, nom, statut, actions propriétaire */}
        <div className="relative bg-white p-6 rounded-xl shadow-md flex flex-col md:flex-row gap-6 items-center md:items-start">
          {/* Bouton d’édition visible uniquement si c’est mon propre profil */}
          {isOwner && (
          ...
          {/* Infos principales : nom, rôle, vérification, localisation, bio, compteurs */}
          <div className="flex-1 space-y-2 md:pt-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {/* Badge de rôle (organisateur vs utilisateur) */}
              <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded">
                👤 {user?.role === 'organisateur' ? 'Organisateur' : 'Utilisateur'}
              </span>

              {/* Indication de compte vérifié si présent dans les données */}
              {user?.verifie && (
            ...
            {/* Compteurs et lien vers la pièce d’identité (propriétaire uniquement) */}
          ...
          {activities.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {activities.map((a) => (
                ...
                >
                  {/* Visuel de couverture de l’activité */}
                  <img
                  ...
                  {/* Détails courts : titre, lieu, première date éventuelle, extrait description */}
